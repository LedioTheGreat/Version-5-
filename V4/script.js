...
document.addEventListener('DOMContentLoaded', () => {
    loadCars();
    initMobileMenu();
    // REMOVED custom contact form JS to restore Formspree redirect
    initCookieNotice();
    initModal();
    initSmoothScrolling();
});
...
function initModal() {
    ...
    elements.carModal.setAttribute('aria-modal', 'true');
    elements.carModal.setAttribute('role', 'dialog');
...
function openCarModal(carId) {
    ...
    const focusableEls = elements.carModalContent.querySelectorAll('button, [href], input, select, textarea, [tabindex]:not([tabindex="-1"])');
    if (focusableEls.length) focusableEls[0].focus();
    elements.carModalContent.tabIndex = -1;
    elements.carModalContent.focus();
}
...
function createModalContent(car) {
    ...
    if (images.length > 0) {
        ...
        imageContent = `
            <div class="slider-container">
                ...
                ${images.length > 1 ? `
                    <button class="slider-arrow left" onclick="prevSlide(${car.id})" aria-label="Previous Image"><i class="fas fa-chevron-left"></i></button>
                    <button class="slider-arrow right" onclick="nextSlide(${car.id})" aria-label="Next Image"><i class="fas fa-chevron-right"></i></button>
                ` : ''}
    ...
    return `<span class="close" tabindex="0" aria-label="Close Modal">&times;</span>
...
function initSlider(carId) {
    ...
    const oldState = sliderStates.get(carId);
    if (oldState && oldState.intervalId) clearInterval(oldState.intervalId);
...
function closeCarModal() {
    sliderStates.forEach(state => state.intervalId && clearInterval(state.intervalId));
    sliderStates.clear();
...
// REMOVED custom contact form JS -- restored default HTML POST to Formspree
